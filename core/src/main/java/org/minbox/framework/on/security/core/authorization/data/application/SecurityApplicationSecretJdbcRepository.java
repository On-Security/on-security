 ...
 * @since 0.0.1
 */
public class SecurityApplicationSecretJdbcRepository extends OnSecurityBaseJdbcRepositorySupport<SecurityApplicationSecret, String>
        implements SecurityApplicationSecretRepository {
    public SecurityApplicationSecretJdbcRepository(JdbcOperations jdbcOperations) {
        super(OnSecurityTables.SecurityApplicationSecret, jdbcOperations);
    }

    @Override
    public void save(SecurityApplicationSecret clientSecret) {
        SecurityApplicationSecret storedApplicationSecret = this.selectOne(clientSecret.getId());
        if (storedApplicationSecret != null) {
            this.update(clientSecret);
        } else {
            this.insert(clientSecret);
        }
    }
